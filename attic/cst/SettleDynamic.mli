(**This module offers a type of concrete syntax trees (CSTs), a type of
   ...
   which converts a DCST to a valid CST.
   ...
   insert parentheses so as to produce a valid CST. A CST [t] is valid if the
   parser's round-trip property holds: [parse (fringe t) = Some t]. *)
...
module Make (A : sig
  ...
  (**A type of terminal symbols. We assume that the subsumption relation
     [terminal :> int] holds; that is, a terminal symbol can be converted
     to an integer. *)
  ...
  (**A type of productions. We assume that the subsumption relation
     [production :> int] holds; that is, a production can be converted
     to an integer. *)
  ...
  (**The module [CST] offers an algebraic data type [cst] of concrete syntax
     ...
     A CST is {i valid} if the parser accepts its fringe and produces the
     exact same CST again. In general, not every CST is valid: as a typical
     ...
     child of a multiplication node is not valid. If the grammar is LR(1) then
     every CST is valid. *)
  module CST : sig
