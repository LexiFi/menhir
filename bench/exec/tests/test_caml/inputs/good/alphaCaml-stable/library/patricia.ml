...
module Make (X : Endianness.S) = struct
  ...
  let add_or_lookup k d m =
    let existing = ref d (* dummy *) in
    try
      Added (
	basic_add (fun d0 _ ->
	  existing := d0;
  ...
  (* [build] is a ``smart constructor''. It builds a [Branch] node
     with the specified arguments, but ensures that the newly created
     node does not have an [Empty] child. *)
  ...
  let rec inter decide s t =
	...
	begin try
	  let d1 = find x s in
	  let d = decide d1 d2 in
	  if d == d2 then t else Leaf (x, d)
	with Not_found ->
	  Empty
	end
    | Leaf (x, d1), _ ->
	...
	if (p = q) & (m = n) then
	  ...
	  let u0 = inter decide s0 t0
	  and u1 = inter decide s1 t1 in
	  if u0 == t0 && u1 == t1 then t else build p m u0 u1

	else if (X.shorter m n) & (match_prefix q p m) then

	  (* [q] contains [p]. Intersect [t] with a sub-tree of [s]. *)

	  inter decide (if (q land m) = 0 then s0 else s1) t

	else if (X.shorter n m) & (match_prefix p q n) then

	  (* [p] contains [q]. Intersect [s] with a sub-tree of [t]. *)

	  inter decide s (if (p land n) = 0 then t0 else t1)
  ...
  (* TEMPORARY performance could be improved, see JCF's paper; same for sets, below *)
  ...
  let subset s t =
    ...
    let rec diff s t =
	  ...
	  if (p = q) & (m = n) then begin
	    ...
	    (* Either [q] contains [p], which means at least one of
	       [s]'s sub-trees is not contained within [t], or the
	       prefixes disagree. In either case, the subset
	       relationship cannot possibly hold. *)
...
(* TEMPORARY reconsider; maps to the unit element would not be so bad *)
...
module Domain = struct
  ...
  (* [disjoint s1 s2] returns [true] if and only if the sets [s1] and
     [s2] are disjoint, i.e. iff their intersection is empty. It is a
     specialized version of [inter], which uses less space. *)
  ...
  (* [subset] implements the subset predicate over sets. In other
     words, [subset s t] returns [true] if and only if $s\subseteq
     t$. It is a specialized version of [diff]. *)
