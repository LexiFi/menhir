...
(* TEMPORARY change syntax of container declarations to provide a
   single module name, which must provide appropriate fields. *)
...
module Container = struct
  ...
  let containers : string list =
    [ "list"; "option" ]

  let containers : (string * string * string * string) StringMap.t =
    List.fold_left (fun containers container ->
      let prefix = prefix ^ container in
      StringMap.add
	container
	(prefix ^ "_map", prefix ^ "_self_map", prefix ^ "_fold", prefix ^ "_fold2")
	containers
    ) StringMap.empty containers

  let containers : (string * string * string * string) StringMap.t ref =
    ref containers

  let add (pos1, pos2, x) (_, _, map) (* (_, _, self_map) TEMPORARY *) (_, _, fold) (_, _, fold2) =
    let self_map = map in (* TEMPORARY making up self_map; should change syntax of container declarations *)
    try
    ...
    with Not_found ->
      containers := StringMap.add x (map, self_map, fold, fold2) !containers
  ...
  let map = function
	...
	""
    | Some (_, _, c) ->
	let map, _, _, _ = StringMap.find c !containers in
	map ^ " "

  let self_map = function
    | None ->
	""
    | Some (_, _, c) ->
	let _, self_map, _, _ = StringMap.find c !containers in
	self_map ^ " "
  ...
  let fold = function
	...
	""
    | Some (_, _, c) ->
	let _, _, fold, _ = StringMap.find c !containers in
	fold ^ " "
  ...
  let fold2 = function
	...
	""
    | Some (_, _, c) ->
	let _, _, _, fold2 = StringMap.find c !containers in
	fold2 ^ " "
...
(* Translate the type definitions to internal syntax, in order
   to make life simpler for the generator. Ideally, the parser
   could perhaps build a simpler structure to begin with. *)
...
let translate_modifier = function
  | Syntax.MInner ->
      InternalSyntax.MInner
  | Syntax.MOuter ->
      InternalSyntax.MOuter
  | Syntax.MNeutral ->
      InternalSyntax.MNeutral
  | Syntax.MRef ->
      assert false

let translate_expfactor = function
  | EAtom sort ->
      FAtom (id2s sort)
  | EEscape t ->
      FEscape t
  | ETypRef (None, params, t) ->
      FTypCon (MNone, id2s t, ids2ss params)
  | ETypRef (Some container, params, t) ->
      FTypCon (MContainer (id2s container), id2s t, ids2ss params)
  | EAbstraction (params, t) ->
      FTypCon (MAbstraction, id2s t, ids2ss params)

let translate_patfactor = function
  | PAtom sort ->
      FAtom (id2s sort)
  | PEscape t ->
      FEscape t
  | PTypRef (MRef, None, params, t) ->
      FTypCon (MNone, id2s t, ids2ss params)
  | PTypRef (MRef, Some container, params, t) ->
      FTypCon (MContainer (id2s container), id2s t, ids2ss params)
  | PTypRef (modifier, None, params, t) ->
      FTypCon (translate_modifier modifier, id2s t, ids2ss params)
  | PTypRef (_, Some _, _, _) ->
      assert false (* TEMPORARY *) (* cannot translate this: restrict input syntax? *)

let translate_labeled_factor translate_factor = function
  | Some label, factor ->
      id2s label,
      translate_factor factor
  | None, _ ->
      assert false

let translate_unlabeled_factor translate_factor = function
  | None, factor ->
      translate_factor factor
  | Some _, _ ->
      assert false

let translate_summands translate_factor summands =
  match summands with
    | [ Summand (None, ((Some _, _) :: _ as factors)) ] ->
	(* This is a record type. *)
	ADT.Record (List.map (translate_labeled_factor translate_factor) factors)
    | [ Summand (None, ((None, _) :: _ as factors)) ] ->
	(* This is a tuple type. *)
	ADT.Tuple (List.map (translate_unlabeled_factor translate_factor) factors)
    | _ ->
	(* This is a sum type. *)
	ADT.Sum (List.map (function
	  | Summand (Some d, factors) ->
	      id2s d,
	      List.map (translate_unlabeled_factor translate_factor) factors
	  | Summand (None, _) ->
	      assert false
        ) summands)

let translate_declaration accu = function
  | DeclExpType (params, (_, _, t), summands) ->
	...
	t
        (DeclType (
          ...
          t,
	  ModeExp,
	  translate_summands translate_expfactor summands
	))
	accu
  | DeclPatType (params, (_, _, t), _, summands) ->
	...
	(DeclType (
	  ...
	  t,
	  ModePat,
	  translate_summands translate_patfactor summands
	))
	accu
  | DeclSort _
  | DeclContainer _
...
type domain =
  | DAll         (* all types *)
  | DAbstraction (* only the types that appear within an abstraction *)

module Declarations = struct

  let dall =
    List.fold_left translate_declaration StringMap.empty Front.declarations

  let dabstraction =
    StringMap.fold (fun t declaration accu ->
      if StringSet.mem t abstractions then
	StringMap.add t declaration accu
      else
	accu
    ) dall StringMap.empty
  ...
  let find domain t =
    StringMap.find t (declarations domain)

  let fold domain f accu =
    StringMap.fold (fun _ declaration accu ->
      f declaration accu
    ) (declarations domain) accu

  let map domain f =
    fold domain (fun declaration accu ->
      f declaration :: accu
    ) []

  let flat_map domain f =
    fold domain (fun declaration accu ->
      f declaration @ accu
    ) []
