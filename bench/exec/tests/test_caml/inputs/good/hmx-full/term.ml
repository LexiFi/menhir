(* $Header: /home/pauillac/formel1/fpottier/cvs/hmx/term.ml,v 1.1 2001/03/21 21:56:30 fpottier Exp $ *)

(* This tiny signature describes the operations which [Herbrand] expects are supported by dimensions and labels. *)
...
module type Algebra = sig
  ...
  (* [print term] returns a list of labeled sub-terms and tokens. [sprint] prints a symbol. [parenthesize label
     subterm] tells whether the given [subterm] must be parenthesized, if found at the given [label] within a larger
     term. [safe label] tells whether a label occurs between two tokens, i.e. subterms at this label \emph{never} need
     to be parenthesized. *)
