(* $Header: /home/pauillac/formel1/fpottier/cvs/hmx/sets/sets.ml,v 1.1.2.1.2.1 2003/01/27 12:42:50 fpottier Exp $ *)
...
module System = Herbrand.Make(RowLabel)(GroundSig)
...
module Primitives = struct
  ...
  let rule mono = function
	...
	let arg2 = arrow (set (juxtapose label present rho)) alpha in
	let arg3 = arrow (set (juxtapose label absent  rho)) alpha  in
	...
	and alpha = fresh()
	and beta = fresh() in
	let arg1 = sum (present1 alpha) psi
	and arg2 = arrow alpha beta
	and arg3 = arrow unit beta in
	[], arrow arg1 (arrow arg2 (arrow arg3 beta))
    | PrimChoice ->
	let alpha = fresh() in
	[ alpha; alpha ], alpha
