...
(** [Ordered] describes an abstract type [t] equipped with a total
    ordering. *)
module type Ordered = sig

  type t

  (** [compare x y] returns a negative integer if [x] is less than
      [y], 0 if [x] and [y] are equal, and a positive integer if [x]
      is greater than [y]. *)
  val compare: t -> t -> int
...
module StringSet = struct 
  ...
  let strict_union s s' =
    let i = inter s s' in
    if i <> empty then
      raise (Strict (List.hd (elements i)))
    else union s s'
...
let invariant cond value = 
  if not cond then (
    Printf.eprintf "Invariant unverified for %s.\n" value;
    flush stderr);
  cond
...
let warning_fun = 
  ref (fun s -> Printf.eprintf "WARNING: %s\n" s)

let set_warning_function f =
  warning_fun := f
