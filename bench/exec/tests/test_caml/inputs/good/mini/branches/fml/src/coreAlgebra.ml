...
let product_variance v1 v2 = 
  match v1, v2 with  
  | InVariant, _ | _, InVariant -> InVariant    (* invariant is absorbant *)
  | CoVariant, v | v, CoVariant -> v            (* covariant is neutral   *)
  | ContraVariant, ContraVariant -> CoVariant   (* - time - is +          *)
...
let tvar var = TVariable var
             
let app t args = 
  List.fold_left (fun acu x -> TTerm (App (acu, x))) t args

let gen tvars subterm = 
  List.fold_left 
    (fun acu var -> TGen (var, acu)) 
    subterm 
    (List.rev tvars)
...
and type_variables term acu = 
  match term with
    ...
    | TGen (x, term) -> x :: (type_variables term acu)
         
let type_variables term = type_variables term []
 

let rec cut_heading_gen = function      
  | TGen (v, sub) ->
      let quantifiers, rest = cut_heading_gen sub in
       v::quantifiers, rest
  | _ as rest -> [], rest

 
let find_gen_vars term = 
  let rec find_rec acu = function
    | TVariable _  -> acu
    | TTerm t -> fold (fun term acu' -> find_rec acu' term) t acu
    | TGen (v, term) -> find_rec (v::acu) term 
  in
  find_rec [] term 

let find_free_vars term = 
  let rec find_rec env = function
    | TVariable x -> if List.memq x env then [] else [x]
    | TTerm t -> fold (fun sub acu -> (find_rec env sub) @ acu) t []
    | TGen (x, sub) -> find_rec (x::env) sub
  in
  find_rec [] term   
  ...
  function
    ...
    | TVariable x as term -> 
        (try 
          List.assq x c
        with Not_found -> term)
    | TGen (v,t) -> TGen (v, change_arterm_vars c t)
...
and gen_change_arterm_vars c = 
    function
      ...
      | TGen (v,t) -> 
          failwith "unsupported for TGen in change_arterm_vars"
