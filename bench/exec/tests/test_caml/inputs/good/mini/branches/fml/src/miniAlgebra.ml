...
type builtin_dataconstructor = string * MiniPst.typ
...
let builtin_env = 
  ...
  in
    [| 
       ...
       *)
      "->", (true, AssocRight, 0, KArrow (KContraVariant, KArrow (KCoVariant, KInVariant)), []);
      "*", (true, NonAssoc, 1, KArrow (KCoVariant, KArrow (KCoVariant, KInVariant)), 
            [ ("_Tuple", 
              TypGen (undefined_position, [ "a1"; "b1" ], 
                arrow_type undefined_position (gen_tvar "a1") 
                 (arrow_type undefined_position (gen_tvar "b1")
                    (tuple_type2 undefined_position 
                       (gen_tvar "a1") (gen_tvar "b1"))))
              )
            ]);
      "int", (false, NonAssoc, 2, KInVariant, []);
      "char", (false, NonAssoc, 2, KInVariant, []);
      "unit", (false, NonAssoc, 3, KInVariant, 
               [ "_Unit", TypVar (undefined_position, "unit") ]
               )
...
let get_infix (i, _, _,_,_) = i

let get_assoc (_, a, _,_,_) = a

let get_priority (_, _, p,_,_) = p

let as_symbol name = 
  Misc.just_try (fun () -> Misc.array_associ name builtin_env)

let init_builtin_env variable =   
  Array.fold_left (fun acu (o, (_,_,_,arity, ds)) -> 
                     (o, (arity, 
  ...
  let rec chop n t = 
    if n = 0 then t 
      ...
      match t with
        ...
        | TGen _ -> failwith "result_type applied on TGen"
        | u -> assert (n <= 0); u
  in
    chop (-1) t
...
let arg_types tenv t = 
  ...
  let rec chop acu = function
    ...
    | TGen _ -> failwith "arg_types applied on TGen"
    | x -> acu
  in
    List.rev (chop [] t) 
