...
(* $Id: check-ocaml-version.ml 3 2007-09-17 09:09:58Z yann.regisgianas $ *)

(** This script parses ocaml version and confronts it with a user-provided
    version. *)
...
let _ =
  match !version with
	...
	if compare_version ov uv then begin
	  ...
	  if !verbose then
	    Printf.printf "%s is NOT OK: version %s %s was required.%!\n" 
