...
let print_clauses channel comment clauses =
  ...
  Error.log 1 (fun channel ->
    fprintf channel "Emitting a SAT problem of %d variables and %d clauses.\n%!"
      vcount ccount
  );
  ...
  let print_variable numeric negated v =
	...
	Var.Atom.basename v
    in
    bprintf buffer "%s%s " (if negated then "-" else "") (p v)
  in
  ...
  if false then (* toggle *)
    for i = 1 to vcount do
      bprintf buffer "c %3d stands for %s\n" i (Var.Atom.basename (reverse i))
    done;
...
let solve comment mode = function
  ...
  | PConj clauses ->
	...
	Filename.open_temp_file "fml" ".cnf"
      ...
      let convert = print_clauses channel comment clauses in
      close_out channel;
      Error.log 1 (fun channel ->
	fprintf channel "Invoking %s... " (command !solver mode)
      );
	...
	if false then (* toggle *)
      ...
      let result =
	IO.winvoke [] command [ cleaner ]
      in

      Error.log 3 (fun channel ->
	output_string channel result
      );
	...
	| ZChaff.Failure
	| Relsat.Failure ->
	    Error.errors [] (sprintf "Internal error (cnf): could not lex:\n%s" result)
      in
