...
(*                                                                                                                   *)
(*********************************************************************************************************************)
(* $Header: /home/pauillac/formel1/fpottier/cvs/wallace/print.ml,v 1.9.2.1 2001/03/19 18:25:25 fpottier Exp $ *)
...
module type GenericPrinter = sig
  ...
  (* [variable quantified sign n] prints a type variable, given its [sign] and its unique number [n]. Numbers range up
     starting from 1. The flag [quantified] tells whether the variable is universally quantified; for clarity, such
     variables may be syntactically distinguished from monomorphic variables. *)

  type sign

  val variable: bool -> sign -> int -> unit
...
(*s \mysection{Printing} *)
  ...
  let rec variable shield is_row lf =
	...
	| Row _, false ->
      ...
      (* TEMPORARY what's the definitive way of telling whether a variable is quantified? *)

      GenericPrinter.variable (lf.rank = !Core.Level.level) lf.sign (Name.variable lf)
  ...
  type scheme = Core.leaf (* TEMPORARY *)

  let scheme substitute pscheme scheme =
    
    (* If requested by the caller, prepare for on-the-fly substitutions. *)

    assert (not substitute); (* TEMPORARY substitutions should affect only quantified/fresh variables *)
