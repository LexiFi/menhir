...
(*                                                                                                                   *)
(*********************************************************************************************************************)
(* $Header: /home/pauillac/formel1/fpottier/cvs/wallace/core.ml,v 1.41 2002/11/15 14:10:35 fpottier Exp $ *)
  ...
  module Minimize = struct
    ...
    module Point = struct
	 ...
	 constraints. *)
      ...
      type point = {
          ...
          (* This field is used by our implementation of Paige and Tarjan's algorithm to store internal data.
	     We should really use an [option] type here, because no values of type [point PaigeTarjan.item]
	     will be available when we initially create [point] objects. However, provided we break the type
	     system, we can also initialize the field with a dummy value. We do so below. *)

	  mutable point_data: point PaigeTarjan.item;
	...
	| _ ->
	 ...
	 use an explicit association structure. *)
      ...
      let of_leaf lf =
	let point = {
	  point_data = Obj.magic None; (* dummy value *)
	  point_leaf = Some lf
	} in
