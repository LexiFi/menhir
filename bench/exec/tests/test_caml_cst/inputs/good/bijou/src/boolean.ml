...
let solve print comment mode = function
  ...
  | FFalse ->
      []
  | f ->
      Cnf.solve print comment mode (cnf (nnf f))

let satisfiable print f =
  match solve print "" Cnf.ModeOneSolution f with
  | [] ->
      false
  | _ :: _ ->
      true
...
let solutions print domain f : solution list =
  ...
  ) [] (solve print "" Cnf.ModeAllSolutions f)
