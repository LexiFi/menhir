...
let rec iterator elem buffer xs =
  match xs() with
  | None ->
      ()
  | Some x ->
      bprintf buffer "%a%a" elem x (iterator elem) xs

let rec preciterator delim elem buffer xs =
  match xs() with
  | None ->
      ()
  | Some x ->
      bprintf buffer "%t%a%a" delim elem x (preciterator delim elem) xs

let rec termiterator delim elem buffer xs =
  match xs() with
  | None ->
      ()
  | Some x ->
      bprintf buffer "%a%t%a" elem x delim (termiterator delim elem) xs
...
(* [ws] turns a buffer printer into a string. *)
