...
module Make (P : sig
  ...
  val print_set_variable: set_variable printer
  val print_boolean_variable: boolean_variable printer
...
let eliminate c =
  Propositional.disjunction_map (fun assignment ->
    ground assignment c
  ) (Boolean.solutions print_boolean_variable (fbv c) (extract c))
...
let satisfiable_simple_constraint po no =
  Boolean.satisfiable print_set_variable (
    Propositional.conjunction (
      match po with
      | None ->
	  Propositional.truth
