    ...
    List.iter (fun v ->
		 let desc = UnionFind.find v in
		   if desc.rank < young_number then
		     ...
		     desc.rank <- Rank.none;
		     if desc.kind = Flexible then 
		       (desc.kind <- Rigid; 
			(* When a flexible variable is generalized, the 
			   user identifier given to it is lost.
			   It is a matter of choice but I think it is 
			   clearer for the programmer. For example, for the
			   program: let id x = exists f. (x : f)
			   I do not want that scheme to be inferred:
			   id : forall f. f -> f
			   But this one:
			   id : forall a. a -> a
			   In other words, the only user names accepted for the
			   generalized type variables are introduced by "forall".
			*)
			desc.name <- None)
		   )
	      ) sorted.(young_number)
...
(*	     raise (NonExhaustiveMatch (pos, pat)) *)
    ...
    and unify_terms pos pool t1 t2 =
      let saved_t1, saved_t2 = twice (fun x -> instance pool x) t1 t2 in
	unify ~tracer:(fun v1 v2 -> 
			 tracer (UnifyVars (v1, v2))
		      )
