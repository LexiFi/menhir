...
struct
  ...
  *)
  ...
  (* The following information is stored for each datatype constructor:
     ...
     - its type ; 
  *)
  type data_constructor = variable list * crterm
  ...
  let tycon_name_conflict pos env (fqs, denv) =
    match filtered_lookup (is_typcon env) (assoc_proj1 denv) with
      ...
      | None -> (fqs, 
		 List.map 
		   (function (n, TVariable v) -> (n, v) | _ -> assert false) 
		   denv)
  ...
  let lookup_datacon ?pos env (k: Algebra.Ast.tname) = 
    ...
    with Not_found -> raise (UnboundDataConstructor ((pos_or_undef pos), k))
      ...
      (* FIXME: not correct ! *)
  let rigid_var_args rt = 
    let args = tycon_args rt in
    (* Variables that are linear. *)
    let var_args = 
      List.fold_left
	(fun acu -> function 
	     TVariable v -> 
	       if (not (is_structured v)) 
		 && (UnionFind.find v).kind = Rigid then 
		   if (List.memq v acu) then
		     List.filter (notf (( == ) v)) acu
		   else 
		     v :: acu
	       else acu
	   | _ -> acu) [] args
    in
    let is_in_term v = 
      List.exists (fun t -> (t <> TVariable v) && List.memq v (type_variables t)) 
	args 
    in
      List.filter (notf is_in_term) (List.rev var_args)
	   
  let fresh_datacon_scheme pos tenv k localvars =
    let (kvars, kt) = lookup_datacon tenv k in 
    let rt          = result_type (as_fun tenv) kt in
    let alphas      = rigid_var_args rt in
    let betas = 
      List.filter (fun v -> not (List.memq v alphas)) kvars in 
    let _ = 
      if localvars <> [] && List.length localvars <> List.length betas then (
	Printf.eprintf "%d %s\n" (List.length betas) k;
	...
	let fvs = List.map2 (fun name -> mkvar ~name) localvars betas in
	  (List.map mkvar alphas @ fvs,
	   List.combine localvars (List.map (fun v -> TVariable v) fvs))
    in
    let fresh_kvars_assoc = List.combine (alphas @ betas) fresh_kvars in
    let _, denv = tycon_name_conflict pos tenv (betas, denv)
    in
    let (alphas, betas) = 
  ...
  let is_regular_datacon_scheme tenv kvars kt =
    let rt = result_type (as_fun tenv) kt in
    let rigid_var_args = rigid_var_args rt in
      (* Check that all the tycon arguments are distinct rigid variables. *)
      List.for_all (fun v -> List.memq v kvars) rigid_var_args 
      && List.length (tycon_args rt) = List.length rigid_var_args

  let is_guarded_datacon_scheme tenv kvars kt =
    not (is_regular_datacon_scheme tenv kvars kt)

  let is_guarded_datacon tenv k =
    let (kvars, t) = lookup_datacon tenv k in
      is_guarded_datacon_scheme tenv kvars t 
  ...
  let fresh_vars kind pos env vars =
    let vs = variable_list_from_strings (fun v -> (kind, Some v)) vars in
