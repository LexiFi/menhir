...
let deps tname =
  (Hashtbl.find registered_tasks tname).depends_on
...
let options tstart tend others = 
  let options, afs = 
...
let options_analysis default_tstart default_tend others = 
  let tstart = ref default_tstart and tend = ref default_tend in 
  let options, anon_funs = options tstart tend others in
    Arg.parse options anon_funs "";
    options, !tstart, !tend
...
let execute ~default_start ~default_end ~usage others = 
  let options, tstart, tend = 
    options_analysis default_start default_end  others in 
