(* $Header: /home/yquem/cristal/fpottier/cvs/modulo/unionFind.ml,v 1.3 2003/04/02 11:15:02 fpottier Exp $ *)
...
(* [self desc] creates a fresh point [point] and returns it. It forms
   an equivalence class of its own, whose descriptor is [desc
   point]. In other words, the descriptor is allowed to recursively
   refer to the point that is being created. Use with caution -- the
   function [desc] is allowed to store a pointer to [point], but must
...
let self desc =
  ...
  point.link <- Info { weight = 1; descriptor = desc point };
  point
...
(* [info point] returns the [info structure associated with [point]'s
   equivalence class. *)
...
let rec find point =
  (info point).descriptor

(* [change point desc] changes the descriptor of [point]'s equivalence
   class to [desc]. [desc] is returned. *)

let change point desc =
  (info point).descriptor <- desc;
  desc
