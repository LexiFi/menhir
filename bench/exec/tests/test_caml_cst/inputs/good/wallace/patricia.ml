...
(*                                                                                                                   *)
(*********************************************************************************************************************)
(* $Header: /home/pauillac/formel1/fpottier/cvs/wallace/patricia.ml,v 1.7 2000/02/11 16:15:50 fpottier Exp $ *)
...
module Endianness = struct
  ...
  module Big = struct
    ...
    let rec highest_bit4 mask x =
      mask lor (Array.unsafe_get table4 x)

    and highest_bit8 mask x =
      if x land 0xF0 <> 0 then
	highest_bit4 (mask lor 4) (x lsr 4)
      else
	highest_bit4 mask x

    and highest_bit16 mask x =
      if x land 0xFF00 <> 0 then
	highest_bit8 (mask lor 8) (x lsr 8)
      else
	highest_bit8 mask x

    and highest_bit31 mask x =
      if x land 0x7FFF0000 <> 0 then
	highest_bit16 (mask lor 16) (x lsr 16)
      else
	highest_bit16 mask x

    let highest_bit x =
      1 lsl (highest_bit31 0 x)
...
module Make (X : Endianness.S) = struct
  ...
  (* [fine_union decide m1 m2] returns the union of the maps [m1] and [m2]. If a key [k] is bound to [x1]
     (resp. [x2]) within [m1] (resp. [m2]), then [decide] is called. It is passed [x1] and [x2], and must return the
     value which shall be bound to [k] in the final map. *)
  ...
  (* [union m1 m2] returns the union of the maps [m1] and [m2]. *)

  let union m1 m2 =
    fine_union (fun d d' -> d' (* for instance *)) m1 m2
  ...
  module Domain = struct
	...
	| Branch (p, m, t0, t1) ->
	    if match_prefix x p m then
	      if (x land m) = 0 then Branch (p, m, add t0, t1)
	      else Branch (p, m, t0, add t1)
	    else
	      join x (Leaf x) p t in
