...
The \verb+Fail_pr_full+ parse result provides the third and simplest option. By including both
state and token information, it is possible to construct meaningful error messages with a small amount
of work. A function \verb+N_of_state+ is provided in the \verb+Aut+ module which takes in a state and
returns the state number. While this is not as powerful as the advanced error handling enabled by the 
incremental API for non-verified parsers, this does allow interoperability with the existing \messages 
files and tooling, described in \sref{sec:messages:format} and \sref{sec:messages:tools}. This allows
error messaging without a second parser. An example is provided in \distrib{demos/coq-syntax-errors}.

Note that the extra information carried by \verb+Fail_pr_full+ (and in the related \verb+Fail_sr_full+ used
internally) is not verified. They are provided for convenience, but not with proofs of their correctness.

Users who wish to ignore this extra information can use the abbreviated notation provided, \verb+Fail_pr+ and 
\verb+Fail_sr+, as used in the statements of the parser theorems. (These are only available in Coq code,
such as in \distrib{demos/coq-minicalc}, not in extracted OCaml). 
% ------------------------------------------------------------------------------
