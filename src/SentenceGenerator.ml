...
let unwrap xs = match xs with [x] -> x | _ -> assert false

(* Uncurried list cons. *)

let cons (x, xs) = unwrap x :: xs

(* [up i j] is the list of the integers of [i] included up to [j] included. *)

let rec up i j = if i <= j then i :: up (i + 1) j else []
...
let trees : query -> int -> cst list seq =
  ...
  let module M = Fix.Memoize.ForType (struct
    type t = query * int
  end) in
  M.(curried fix) (fun trees query budget ->
      match query with
      (* Query: what are the trees generated by the symbol [nt]? *)
      | QNT nt ->
          if budget < Analysis.minimal nt then empty
          else
            ...
            Production.foldnt nt
              (fun prod accu ->
                if
                  ...
                  && Analysis.minimal_prod prod 0 <= budget
                then accu ++ trees (QProd prod) budget
                else accu)
              empty
      ...
      | QProdSuffix (prod, i) -> (
          ...
          if i = n then
            ...
            | Symbol.N nt ->
                ...
                if is_terminal_suffix rhs i then (
