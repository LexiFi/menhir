...
let assign p v = assign empty p v
...
let let_and bs1 bs2 =
  assert (RegisterSet.disjoint (domain bs1) (domain bs2));
  RegisterMap.union (fun _ -> assert false) bs1 bs2
...
let partition bs regs =
  RegisterMap.partition
    (fun _ v ->
      let vregs = Value.registers v in
      RegisterSet.is_empty (RegisterSet.inter regs vregs) )
    bs
