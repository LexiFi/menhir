...
module Run (T : sig end) = struct
  ...
  let goto nt = prefix (Printf.sprintf "goto_%s" (Nonterminal.print true nt))

  (* The [reduce] function associated with a production [prod]. *)

  let reduce prod = prefix (Printf.sprintf "reduce%d" (Production.p2i prod))
  ...
  let assertnoerror : pattern * expr =
    (PUnit, eassert (EApp (EVar "not", [ERecordAccess (EVar env, ferror)])))
  ...
  let tracecomment (comment : string) (body : expr) : expr =
    if Settings.trace then blet (trace comment [], body)
    else EComment (comment, body)
...
(* ------------------------------------------------------------------------ *)
  ...
  let celltype tailtype holds_state symbol _ =
    TypTuple
      ( (tailtype :: if1 (Invariant.endp symbol) tposition)
      @ if1 holds_state tstate @ semvtype symbol
  ...
  let stacktype s = Invariant.fold celltype ttail (Invariant.stack s)

  let reducestacktype prod =
    Invariant.fold celltype ttail (Invariant.prodstack prod)

  let gotostacktype nt = Invariant.fold celltype ttail (Invariant.gotostack nt)
  ...
  let reduce_expects_state_param prod =
    ...
    Production.length prod = 0
    && Invariant.fold
         (fun _ holds_state _ _ -> holds_state)
  ...
  let errorcasetypescheme = auto2scheme (marrow [tenv; ttail; tstate] tresult)
  ...
  let letunless e x e1 e2 =
    EMatch
      ...
      , [branch (PData ("Some", [PVar x])) e1; branch (PData ("None", [])) e2]
      )
  ...
  let action_may_refer_to_value prod i =
    ...
    ||
    let ids = Production.identifiers prod and action = Production.action prod in
  ...
  let errorcellparams (i, pat) holds_state symbol _ =
    ( i + 1
    , ptuple
        ( (pat :: if1 (Invariant.endp symbol) PWildcard)
        @ if1 holds_state (if i = 0 then PVar state else PWildcard)
  ...
  let gotoparams nt : xparams =
    xvar env :: xvar stack :: runcellparams (Invariant.gotostack nt)

  let call_goto nt = EApp (EVar (goto nt), xparams2exprs (gotoparams nt))

  (* Calls to [errorcase]. *)

  let errorcaseparams : xparams = [xvar env; xmagic (xvar stack); xvar state]

  let call_errorcase = EApp (EVar errorcase, xparams2exprs errorcaseparams)
  ...
  let shiftbranch s tok s' =
    ...
    branch (tokpat tok (PVar semv)) (shiftbranchbody s tok s')
  ...
  let runpushcellunless shiftreduce s e =
    if shiftreduce then EComment ("Not allocating top stack cell", e)
  ...
  let gettoken s defred e =
    ...
    | Some (Symbol.N _), None ->
        ...
        if Invariant.errorpeeker s then (
          incr errorpeekers ;
          EIfThenElse
          ...
          blet
            ([assertnoerror; (PVar token, ERecordAccess (EVar env, ftoken))], e)

  (* This produces the header of a [run] function. *)

  let runheader s body =
    let body = tracecomment (Printf.sprintf "State %d:" (Lr1.number s)) body in
    { valpublic= false
    ; valpat= PVar (run s)
  ...
  let initiate s = blet ([assertnoerror], errorbookkeeping (call_error s))
  ...
  let rundef s : valdef =
    ...
    | None ->
        ...
        and reductions = TerminalMap.remove Terminal.error (Lr1.reductions s) in
        ...
        let covered, branches =
          ProductionMap.fold
            (fun prod toks (covered, branches) ->
        ...
        let covered, branches =
          SymbolMap.fold
            (fun symbol s' (covered, branches) ->
              match symbol with
              | Symbol.T tok ->
                  (* There is a shift transition for this token. *)
                  (TerminalSet.add tok covered, shiftbranch s tok s' :: branches)
              | Symbol.N _ ->
                  (covered, branches))
            transitions (covered, branches)
        in
        let branches =
          if TerminalSet.subset TerminalSet.universe covered then branches
          else branches @ [branch PWildcard (initiate s)]
        in
        ...
        runheader s
          (runpushcell s (gettoken s None (EMatch (EVar token, branches))))
  ...
  let reducebody prod =
    ...
    let (_ : int), pat =
      Invariant.fold
        (fun (i, pat) holds_state symbol _ ->
          ( i + 1
          , if i = length - 1 && shiftreduce prod then pat
            else ptuple (pat :: reducecellparams prod i holds_state symbol) ))
    ...
    let extract x =
      ...
      (PTuple [PWildcard; PVar x], EMagic (EVar stack))
    in
  ...
  let reducedef prod =
    ...
    ; valval=
        EAnnot (EFun (reduceparams prod, reducebody prod), reducetypescheme prod)
    }
  ...
  let gotobody nt =
    ...
    | [branch] ->
        (* If there is only one branch, no case analysis is required. This
           ...
           does not have GADTs. *)
        EPatComment ("State should be ", branch.branchpat, branch.branchbody)
    | _ ->
  ...
  let gotodef nt =
    ...
    ; valval=
        EAnnot
          ( EFun (xparams2pats (gotoparams nt), gotopushcell nt (gotobody nt))
          , gototypescheme nt ) }
  ...
  let errorbody s =
    ...
    | s' ->
        ...
        handle s (shiftbranchbody s Terminal.error s')
    | exception Not_found -> (
  ...
  let errordef s =
    ...
    ; valpat= PVar (error s)
    ; valval=
        EAnnot (EFun (xparams2pats errorparams, errorbody s), errortypescheme s)
    }
  ...
  let errorcasedef =
    ...
    ; valval=
        EAnnot
          ( EFun (xparams2pats errorcaseparams, EMatch (EVar state, branches))
          , errorcasetypescheme ) }
  ...
  let entrydef s =
    ...
    let lexer = "lexer" and lexbuf = "lexbuf" in
    let initial_stack =
  ...
  let assertfalsedef =
    ...
    ; valval=
        EAnnot
          ( EFun
              ( [PUnit]
              , blet ([(PUnit, eprintf internal_failure [])], eassert efalse) )
  ...
  let printtokendef =
    destructuretokendef print_token tstring false (fun tok ->
        EStringConst (Terminal.print tok))
          ...
          @ [ SITypeDefs [envtypedef; statetypedef]
            ...
            ; SIValDefs
                ...
                , ProductionMap.fold
                    ...
                    Lr1.entry
                    (Lr1.fold
                       (fun defs s -> rundef s :: errordef s :: defs)
                       (Nonterminal.foldx
                          (fun nt defs -> gotodef nt :: defs)
                          (Production.fold
                             (fun prod defs ->
                               if
