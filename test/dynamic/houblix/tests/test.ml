(* This script creates [dune.auto.gen] files in this directory and in
   each of the subdirectories [*.tests]. *)
...
let () =
  ...
  |> List.iter (fun dir ->
         ...
         let dune_file = open_out (dir ^ "." ^ "dune.auto.gen") in
         header dune_file;
         tests
         |> List.iter (fun test ->
                backends
