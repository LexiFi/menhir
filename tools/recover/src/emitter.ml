...
module Codesharing (S : SYNTHESIZER) (R : RECOVERY with module G = S.G) : sig
  ...
  type instr =
    | IRef of var
    | IAbort
    | IReduce of S.G.production
    | IShift  of S.G.symbol
  ...
  let share actions =
    ...
    begin
      ...
      let rec iter_list = function
        ...
        | (x :: xs) as xxs ->
          ...
          | occurrences, _order -> incr occurrences
          | exception Not_found ->
            Hashtbl.add occurrence_table xxs (ref 1, !order);
            incr order; iter x; iter_list xs
      and iter = function
    ...
    let bindings =
      let register actions (occurrences, order) to_share =
        if !occurrences > 1
        then (order, actions) :: to_share
        else to_share
      in
      let to_share = Hashtbl.fold register occurrence_table [] in
      let order_actions (o1, _) (o2, _) = compare o2 (o1 : int) in
  ...
  and compile_seq ~sharing = function
    | [] -> []
    | x :: xs -> compile_one ~sharing x @ share_seq ~sharing xs
...
sig
  ...
  let emit_depth ppf =
    ...
    Lr1.iter (fun st ->
        let recovery = R.recover st in
        fprintf ppf "%d;" recovery.prefix
      );
    fprintf ppf "|]\n\n"

  module C = Codesharing(S)(R)
