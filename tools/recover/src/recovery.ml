...
module type RECOVERY = sig
  ...
  (** [prefix] is the size of the known prefix of the stack.
      It means that in the kernel of current state, there is an item whose dot is at position [prefix].
      (we know the incoming symbols for these stack frames and we can enumerate the possible state numbers).

      [cases] is a mapping that associates to each possible state found at stack.[-prefix]
      (or None if the stack is empty) a list of reductions to execute.

      The actual list of actions to reduce an item [(state, prod, pos)] is given by
...
struct
  ...
  type trace = Trace of { cost : float; items : item list }

  module Trace = struct
    type t = trace
    let min tr1 tr2 =
      arg_min_float (fun (Trace t) -> t.cost) tr1 tr2

    let cat (Trace tr1) (Trace tr2) =
      Trace { cost = tr1.cost +. tr2.cost; items = tr1.items @ tr2.items }
  end
  ...
  let synthesize =
    ...
    let add_item cost item stack =
      ...
      if cost = infinity then stack else
