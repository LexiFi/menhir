...
module type SYNTHESIZER = sig
  ...
  *)
  ...
  val solve : variable -> float * action list
  ...
  *)
  ...
  let cost_of = function
    ...
    | Tail (st, prod, pos) ->
        ...
        if penalty = infinity then
          ...
          let can_reduce =
            List.exists (fun (_,prods) -> List.mem prod prods) (Lr1.reductions st)
          in
          const (if can_reduce then (cost_of_prod prod, [Reduce prod]) else (infinity, [Abort]))
        else
          let head =
            let sym, _, _ = (Production.rhs prod).(pos) in
            let cost = cost_of_symbol sym in
            if cost < infinity then const (cost, [Shift sym])
            else match sym with
              | T _ -> const bottom
              | N n -> var (Head (st, n))
          in
          ...
          (fun v ->
            ...
            (costh +. costt, Seq actionh :: actiont)
          )
  ...
  let solve =
    let module Solver = Fix.Fix.ForType (struct
        ...
        type property = float * action list
        let bottom = (infinity, [Abort])
        let equal (x, _ : property) (y, _ : property) : bool = (x = y)
        let is_maximal (x, _ : property) = (x = 0.0)
      end)
    in
    Solver.lfp cost_of
  ...
  let report ppf =
    ...
    let solutions = Lr1.fold
        (fun st acc ->
          match List.fold_left (fun (item, (cost, _ as solution)) (prod, pos) ->
              let cost', _ as solution' = solve (Tail (st, prod, pos)) in
              if cost' < cost then (Some (prod, pos), solution') else (item, solution)
    ...
    List.iter (fun (item, states) ->
        ...
        List.iter (fun ((cost, actions), states) ->
            fprintf ppf "at cost %.02f from states %a:\n%a\n\n"
              cost
              (Utils.pp_list (fun ppf st -> fprintf ppf "#%d" (Lr1.to_int st))) states
